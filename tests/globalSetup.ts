...
import { debugPageState } from '../utils/screenshotDebugger.js';
...
before(async function () {
  ...
  if (!baseUrl || !username || !password) {
    throw new Error(
      'Missing required environment variables. Please run `npm run dev` and populate new .env file with desired credentials.',
    );
  }
...
afterEach(async function () {
  if (this.currentTest && this.currentTest.state === 'failed') {
    try {
      const driver = (global as any).driver;
      if (driver) {
        await debugPageState(driver, this.currentTest.fullTitle(), this.currentTest.err as Error);
        console.log('Saved failure artifacts to ./.screenshots');
      }
    } catch (e) {
      console.log(
