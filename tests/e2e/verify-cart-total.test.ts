...
describe('Cart Total Verification E2E Tests', function () {
  ...
  it('Should add all products to cart via API and verify cart total in Front End', async function () {
    ...
    if (!nexus6ProductId || !macbookProductId || !asusProductId) {
      throw new Error(
        'All product ID environment variables are required: NEXUS_6_PRODUCT_ID, MACBOOK_PRO_PRODUCT_ID, ASUS_FULL_HD_PRODUCT_ID',
      );
    }
    ...
    const addResults = await addMultipleProductsToCart(
      apiClient,
      productIds,
      sessionInfo.tokenCookie,
    );
    ...
    assert.strictEqual(
      cartResult.isMatch,
      true,
      `Cart total mismatch! Calculated: ${cartResult.calculatedTotal}, Displayed: ${cartResult.displayedTotal}`,
    );
